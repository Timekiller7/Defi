...
contract Mlm {
    ...
    function newDeposit(address _ref) public payable {
        ...
        || investors[msg.sender].deadline + 1 days < nowTime) {
            ...
            if (investors[msg.sender].deadline + 1 days < nowTime) {
            ...
            investors[msg.sender].deadline = time + 39 days;
            nowTime = time + 10 minutes;
            investors[msg.sender].profit = (amount / 100) * DAILY_PROFIT_PERC;  // за 1 день
            investors[msg.sender].deposited = amount;
        } else {
            if (investors[msg.sender].deadline < nowTime) // по истечении 40 дней можно забрать
                revert("The deposit expires in a day");
                   // для ежедневного профита
            if (checkDaysWithoutReward() > 1) {
