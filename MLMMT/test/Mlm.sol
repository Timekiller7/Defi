...
contract Mlm is ReentrancyGuard{
    ...
    function checkDaysWithoutReward(uint256 index) public view checkIndex(index) returns(uint256) {
        ...
        if (deadline < nowTime) {
            ...
            _days = (nowTime - lastUpdate) / (1 days);
        }
    ...
    function _reinvest(uint256 index) private {
        ...
        uint256 reinvested;
        uint256 send;
        if (invest.deadline >= nowTime) {   // deadline - время последнего начисления
        ...
        if (invest.deadline + 1 days <= nowTime &&
            invest.deposited != 0)
        {
            returnDeposit(index);
        }
    ...
    function calculateReward(uint256 index) public returns(uint256) {  // публична для теста
    ...
    function returnDeposit(uint256 index) private checkIndex(index) {      // конец 40-ка дневного депозита
...
/////// TEST ///////
    function increaseLastUpdate(uint256 _days, uint256 _minutes, uint256 index) public {
        invests[msg.sender][index].lastUpdate += _days * 1 days + _minutes * 1 minutes;
    }
